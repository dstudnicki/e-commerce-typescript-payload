...
function Calendar({ className, classNames, showOutsideDays = true, ...props }: CalendarProps) {
    return (
        <DayPicker
            ...
            className={cn("hidden sm:block p-3", className)}
            classNames={{
                ...
                nav_button: cn(buttonVariants({ variant: "outline" }), "h-9 w-9 bg-transparent p-0 opacity-50 hover:opacity-100"),
                nav_button_previous: "absolute left-1",
                nav_button_next: "absolute right-1",
                table: "w-full border-collapse space-y-1",
                head_row: "flex justify-center",
                head_cell: "text-muted-foreground rounded-md w-9 font-normal text-[1rem]",
                ...
                day: cn(buttonVariants({ variant: "ghost" }), "h-12 w-12 p-0 text-lg font-normal aria-selected:opacity-100"),
